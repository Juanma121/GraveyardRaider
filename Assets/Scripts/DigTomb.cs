...
public class DigTomb : MonoBehaviour {
	...
	IEnumerator OnTriggerEnter2D(Collider2D other){
		if (other.tag == "Player" && Input.GetKey ("q")) { //Evalua la expresion en cuanto colisiona por lo tanto hay que presionar Q antes de tocar la tumba
			...
			if (this.tag == "Tumba_hormigon" && !other.GetComponent<Inventory> ().getPico ())
				yield break;
			...
			//Muevo al jugador a una posicion centrada
			Vector3 reposicionamiento = new Vector3(transform.position.x, transform.position.y -0.3f, transform.position.z);
			other.transform.position = reposicionamiento;

			//Animación de cavar
			other.GetComponent<Animator>().Play("Player_Dig");
			...
			other.GetComponent<Animator>().Play("Player_Idle");
